...
def main():
    ...
    if config.get('cross_valid', False):
        cv_dl = CrossValidDataloaders(config['data'])
        test_metrics = executor.cross_valid(cv_dl, num_classes, class_names_l2, class_names_l3, results_dir)
    else:
        train_test_dl = TrainTestDataLoaders(config['data'])
        test_metrics = executor.train_test(train_test_dl, num_classes, class_names_l2, class_names_l3, results_dir)
